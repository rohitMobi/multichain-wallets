...
class IFrame {
  ...
  createTab(title, link, uniqueName, autoOpen) {
    ...
    const newNavItem = `<li className="nav-item" role="presentation"><a href="#" className="btn-iframe-close" data-widget="iframe-close" data-type="only-this"><i className="fas fa-times"></i></a><a className="nav-link" data-toggle="row" id="${navId}" href="#${tabId}" role="tab" aria-controls="${tabId}" aria-selected="false">${title}</a></li>`
    $(SELECTOR_TAB_NAVBAR_NAV).append(unescape(escape(newNavItem)))

    const newTabItem = `<div className="tab-pane fade" id="${tabId}" role="tabpanel" aria-labelledby="${navId}"><iframe src="${link}"></iframe></div>`
