...
var editor = renderer.create('<div className="note-editor note-frame card"></div>');
var toolbar = renderer.create('<div className="note-toolbar card-header" role="toolbar"></div>');
...
var editable = renderer.create('<div className="note-editable card-block" contentEditable="true" role="textbox" aria-multiline="true"></div>');
...
var buttonGroup = renderer.create('<div className="note-btn-group btn-group"></div>');
var dropdown = renderer.create('<div className="note-dropdown-menu dropdown-menu" role="list"></div>', function ($node, options) {
...
var dropdownCheck = renderer.create('<div className="note-dropdown-menu dropdown-menu note-check" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
...
var dialog = renderer.create('<div className="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>', function ($node, options) {
  if (options.fade) {
    $node.addClass('fade');
  }
...
var popover = renderer.create(['<div className="note-popover popover in">', '<div className="arrow"></div>', '<div className="popover-content note-children-container"></div>', '</div>'].join(''), function ($node, options) {
...
var summernote_bs4_checkbox = renderer.create('<div className="form-check"></div>', function ($node, options) {
...
var ui = function ui(editorOptions) {
  return {
    ...
    palette: function palette($node, options) {
      return renderer.create('<div className="note-color-palette"></div>', function ($node, options) {
        var contents = [];
    ...
    button: function button($node, options) {
      return renderer.create('<button type="button" className="note-btn btn btn-light btn-sm" tabindex="-1"></button>', function ($node, options) {
