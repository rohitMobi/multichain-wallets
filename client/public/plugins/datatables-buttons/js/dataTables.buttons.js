...
$.extend( Buttons.prototype, {
		...
		for ( var i=0, ien=buttons.length ; i<ien ; i++ ) {
			...
			if ( built.conf.buttons || built.conf.split ) {
				...
				if(built.conf.split) {
					...
					$(built.node).append($('<span className="dt-down-arrow">'+this.c.dom.splitDropdown.text+'</span>'))
				}
	...
	_buildButton: function ( config, inCollection, isSplit, inSplit )
	{
		...
		if(isSplit) {
			...
			var dropButton = $('<button className="' + this.c.dom.splitDropdown.className + ' dt-button"><span className="dt-btn-split-drop-arrow">'+this.c.dom.splitDropdown.text+'</span></button>')
				.on( 'click.dtb', function (e) {
					e.preventDefault();
					e.stopPropagation();
	...
	_popover: function ( content, hostButton, inOpts, e ) {
		...
		if ( options.popoverTitle ) {
			display.prepend('<div className="dt-button-collection-title">'+options.popoverTitle+'</div>');
		}
		...
		if (options.closeButton) {
			display.prepend('<div className="dtb-popover-close">x</div>').addClass('dtb-collection-closeable')
		}
